...
#%%
    ...
    def singlePointExtraction(self, aperture_type=0, convolve=False, parameters_file = True, user_ra=0, user_dec=0,\
        ...
        aperture_correction = meta[0]['aperture_correction']
        ...
        for qqq in range(len(df_res)):
            ...
            params_name = path+filenames[qqq]+'_params_file'+'.txt'
            ...
            t = self.customFITSWriter([df_res[qqq]], False, outname,[pec1d], aperture_correction,  namesList, overwrite=True)  
...
#%%
  ...
  #%%Load Real Data
        ...
        files_PSF = os.listdir(current_path+"/PSF/")
        ...
        [PSF_all, pixel_scale, base_l_list,base_r_list,output_file_name, zzz] = user.getSubCubes(PSF_path,user_rs_arcsec,point_source,  PSF, centering, background,r_in,width,lambda_ap,aperture_type, read_only, files_PSF, convolve)
        ...
        if centering:
            ...
            #new_sky,l_c=  preprocess.lambdaBasedCentering(PSF_all, l_c, 0., 0.) ######## TDS

            new_sky,l_c=  preprocess.lambdaBasedCentering(realData_all, l_c, user_ra, user_dec) ######## TDS
        ...
        for j in range(len(PSF_all[0].rs[0])):          #for every radius
                 ...
                 if cubesNames[i] in dct:        # if the datacube is avaliable
                     ...
                     if cubesNames[i+1] in dct:  # if we can calculate the stittching ratio
                         ...
                         if aperture_correction: #if PSC, use stich corrected spectrum
                             ...
                             data.stiched_spectrum.append(stitched_flux)
             ...
             data = dct[cubesNames[len(cubesNames)-1]]     

             if aperture_correction:

                 data.stiched_spectrum.append(np.array(data.spectrum_PSF_corrected)[j,:])

                 data.stiched_error.append(np.array(data.error_PSF_corrected)[j,:])

             else:

                 data.stiched_spectrum.append(np.array(data.corrected_spectrum)[:,j])

                 data.stiched_error.append(np.array(data.error)[:,j])
  ...
  #%%          
        ...
        #%%Create DF
            ...
            #%%
             ...
             plt.savefig(path+"flux_and_err_spectrum_"+str(user_rs_arcsec[j])+"as.png")
             ...
             file_naming = 'last_result_'+str(user_rs_arcsec[j])+'as'
...
#%%
    ...
    def customFITSWriter(self, df_res, PSC, filename, spec1ds, aperture_correction, band_names, overwrite=False):
...
#%%
    ...
    def gridExtraction(self, point_source = False,lambda_ap=5,  centering = False, lambda_cent = 5, parameters_file = False, plots=False, first_subband = 'G140H', last_subband = 'ch_4_LONG', x_steps = -1, y_steps = -1, r = -1, distance = -1, user_ra = 0, user_dec = 0,  user_center=False, aperture_correction = False, convolve=False):
        ...
        #%% create the read only list 
         ...
         if parameters_file:
             ...
             if 'm' in all_grid_params[6] and 'm' in all_grid_params[7]:

                 from astropy.coordinates import SkyCoord

                 Stringc = SkyCoord(all_grid_params[6], all_grid_params[7], frame='icrs')
             ...
             user_center = all_grid_params[8].replace(" ",'') == 'True'
...
#%% Load PSF   
         ...
         if point_source or convolve:
            ...
            print('PSF folder containt these files: !!!' , PSF_files)
        ...
        #%% Centering Process
         ...
         if user_center:
              ...
              if centering:
                ...
                new_sky,lambda_cent =  preprocess.lambdaBasedCentering(realData_all,lambda_cent, user_ra, user_dec) #center by labda, using the 11x11 box
...
#%% PQD
        ...
        #%%
             ...
             if aperture_correction:
                 ...
                 data.stiched_spectrum.extend(np.array(data.spectrum_PSF_corrected))

                 data.stiched_error.extend(np.array(data.error_PSF_corrected))

             else:

                 data.stiched_spectrum.extend(np.array(data.corrected_spectrum)[0,:])

                 data.stiched_error.extend(np.array(data.error)[0,:])
 ...
 #%%                
         ...
         output_file_name = results_path+'last_result_'+str(i)+"_Grid_spec1d.fits"

         t = self.customFITSWriter(all_DFs, False, output_file_name,spec1ds, aperture_correction, all_names, overwrite=True)  
